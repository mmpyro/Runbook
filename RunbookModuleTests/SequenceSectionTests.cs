...
namespace RunbookModuleTests
{
    ...
    public class SequenceSectionTests
    {
        ...
        public void ShouldRetryChapterWhenItFailByTheFirstTime()
        {
            ...
            var chapter = new Chapter("chapter1", ScriptBlock.Create(""), _factory, _logger);
        ...
        public void ShouldReturnsFailStatusCodeWhenRetriesMoreThanNumberOfRetries()
        {
            ...
            var chapter = new Chapter("chapter1", CreateScriptBlock(), _factory, _logger);
        ...
        public void ShouldReturnsFailStatusCodeWhenRetriesMoreThanNumberOfRetriesWithIgnoreStream()
        {
            //Arrange
            var section = new SequenceSection("");
            var chapter = new Chapter("chapter1", CreateScriptBlock(), _factory, _logger, true);
            _psWrapper.HadErrors.Returns(x => true);
        ...
        public void ShouldThrowsArgumentExceptionWhenChapterInCollectionIsNull()
        {
            ...
            var ex = Assert.Throws<ArgumentException>(() =>
                  {
                      section.AddRange(new[] { new Chapter("", CreateScriptBlock(), _factory, _logger), new Chapter("", CreateScriptBlock(), _factory, _logger), null });
                  });
            Assert.That(ex.Message, Is.EqualTo($"Chapter cannot be null. Section: {sectionName}."));
        }
        ...
        public void ShouldSectionSizeBeEqualNumberOfChapters()
        {
            //Arrange
            var section = new SequenceSection("");
            section.Add(new Chapter("1", CreateScriptBlock(), _factory, _logger));
            section.Add(new Chapter("2", CreateScriptBlock(), _factory, _logger));
            section.AddRange(new[] { new Chapter("3", CreateScriptBlock(), _factory, _logger), new Chapter("4", CreateScriptBlock(), _factory, _logger) });
            //Act
